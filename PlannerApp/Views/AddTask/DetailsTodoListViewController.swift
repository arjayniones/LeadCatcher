...
class DetailsTodoListViewController: ViewControllerProtocol,LargeNativeNavbar {
    ...
    var naviFlag:String = "ToDo"; // bcos contactDetail become present mode so in todoDetail need to handle diff present mode
    // when naviFlag ==  ToDo mean is it from ToDoListView or other so it is using push and pop
    // when naviflag == Contact then mean it is from ContactDetail so it is used present and dismiss method
    // diff naviFlag oso impact snapkit setting.
...
//        self.textView.text = convertDateToString();
    ...
    */
    ...
    @objc func save() {
        // flag = New mean insert data
        // flag = Update mean edit data
        // uuid used to uodate local notification
        ...
        {
            self.viewModel.saveSchedule(flag:"Update", uuid:(self.viewModel.addNoteModel?.addNote_ID)!, completion: { val in
                if val == "true" {
    ...
    @objc func saveData()
    {
        // flag = New mean insert data
        // flag = Update mean edit data
        // uuid = "" bcos when insert uuid is auto generated by new instance
        self.viewModel.saveSchedule(flag:"New", uuid: "",  completion: { val in
            if val == "true" {
                //
                ...
                let alert = UIAlertController.alertControllerWithTitle(title: "error".localized, message: val)
                self.present(alert, animated: true, completion: nil);
            }
        })
    }
    ...
    override func updateViewConstraints() {
        ...
        if !didSetupConstraints {
            ...
            if naviFlag == "Contact" // from contactDetail then go in here
            {
